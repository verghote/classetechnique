...
class Jeton {
    ...
    public static function verifier() {
        ...
        if (!isset($_SESSION['token'])) {
            Erreur::envoyerReponse("Le jeton de vérification n'est pas présent sur le serveur.", 'system');
            return false;
        }
        if (!isset($_REQUEST['token'])) {
            Erreur::envoyerReponse("Le jeton de vérification n'a pas été transmis.", 'system');
            return false;
        }
        if ($_SESSION['token'] !== $_REQUEST['token']) {
            Erreur::envoyerReponse("Absence d'un jeton valide vous autorisant à réaliser cette opération.", 'system');
            return false;
        }
        return true;
    }
